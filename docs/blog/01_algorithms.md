...
When we first built the "Editor" module, it was less like a printing press and more like a very tired clerk trying to read every newspaper in the world at once. We found two rotting carcasses eating our CPU cycles.
...
## Strategy B: The Jaccard Hatchet (Algorithm)
...
This was the *piece de resistance*. `difflib` is accurate but slow (The Scalpel). Python `sets` are stupid but fast (The Hatchet).

Before running the expensive DNA test (`difflib`), we now do a "Jaccard Index" check (Set Intersection).
...
**Result:** We eliminated ~95% of the expensive comparisons. We fold the chaos into order, fast.
